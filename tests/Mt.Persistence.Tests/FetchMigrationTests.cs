...
using Mt.Results;
using Xunit;
using LocksSource = Mt.Persistence.Stages.LocksSource;
using Transforms = Mt.Persistence.Stages.Transforms;
using UnlocksSource = Mt.Persistence.Stages.UnlocksSource;
...
public class FetchMigrationTests(PostgresFixture fixture)
{
    ...
    private async Task<Result<Mt.Domain.Stages.LocksSource.IFetchMigration.Response>> ForwardAsync(Id id)
    {
        await using var db = fixture.CreateDbContext();
    ...
    private async Task<Result<Mt.Domain.Stages.UnlocksSource.IFetchMigration.Response>> TeardownAsync(Id id)
    {
        await using var db = fixture.CreateDbContext();
    ...
    private async Task<Result<Mt.Domain.Stages.Transforms.IFetchMigration.Response>> TransformsAsync(Id id)
    {
        await using var db = fixture.CreateDbContext();
    ...
    public async Task Forward_slice_maps_working_state_to_proceed()
    {
        ...
        Assert.IsType<Mt.Domain.Stages.LocksSource.IFetchMigration.Response.Proceed>(result.Unwrap());
    }
    ...
    public async Task Forward_slice_does_not_proceed_when_already_locked()
    {
        ...
        Assert.IsType<Mt.Domain.Stages.LocksSource.IFetchMigration.Response.DoNotProceed>(result.Unwrap());
    }
    ...
    [Fact]
    public async Task Forward_slice_fails_on_a_state_the_stage_must_never_see()
    {
    ...
    public async Task Transforms_slice_proceeds_only_for_prepared()
    {
        ...
        Assert.IsType<Mt.Domain.Stages.Transforms.IFetchMigration.Response.Proceed>(result.Unwrap());
    }
    ...
    public async Task Teardown_slice_proceeds_for_unlocking_and_cancelling(MigrationState state)
    {
        ...
        Assert.IsType<Mt.Domain.Stages.UnlocksSource.IFetchMigration.Response.Proceed>(result.Unwrap());
    }
    ...
    public async Task Teardown_slice_does_not_proceed_when_already_unlocked(MigrationState state)
    {
        ...
        Assert.IsType<Mt.Domain.Stages.UnlocksSource.IFetchMigration.Response.DoNotProceed>(result.Unwrap());
    }
    ...
    [Fact]
    public async Task Teardown_slice_fails_on_a_state_the_stage_must_never_see()
    {
        var id = await SeedAsync(MigrationState.Created);
