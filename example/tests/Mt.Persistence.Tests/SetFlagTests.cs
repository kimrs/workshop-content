...
using Xunit;
using SetLockedResponse = Mt.Domain.Steps.LocksSource.ISetSourceLocked.Response;
using SetUnlockedResponse = Mt.Domain.Steps.UnlocksSource.ISetSourceUnlocked.Response;
...
public class SetFlagTests(PostgresFixture fixture)
{
    ...
    public async Task Last_setup_flag_performs_the_prepared_transition_and_reports_it()
    {
        ...
        await using var db = fixture.CreateDbContext();
        var result = await new Steps.LocksSource.SetSourceLocked(db).HandleAsync(id, default);
        await db.SaveChangesAsync();
    ...
    public async Task Last_unlock_on_the_approve_path_reports_complete()
    {
        ...
        await using var db = fixture.CreateDbContext();
        var result = await new Steps.UnlocksSource.SetSourceUnlocked(db).HandleAsync(id, default);
        await db.SaveChangesAsync();

        var complete = Assert.IsType<SetUnlockedResponse.Complete>(result.Unwrap().Value);
    ...
    public async Task Last_unlock_on_the_cancel_path_reports_cancel()
    {
        ...
        var cancel = Assert.IsType<SetUnlockedResponse.Cancel>(result.Unwrap().Value);
    ...
    public async Task Earlier_unlock_reports_teardown_incomplete()
    {
        ...
        Assert.IsType<SetUnlockedResponse.TeardownIncomplete>(result.Unwrap().Value);
        Assert.True((await LoadRowAsync(id)).SourceUnlocked);
    }
}
