...
/// <summary>
/// Unlocks Source during teardown: a fault at Source is an outcome, answered with a retry
/// (spec 11). Whichever unlock finishes last fans in to the terminal state and notifies
/// completion (§6.5). Intentional near-duplicate of <c>UnlocksTarget</c> (§4.3). Runs for
...
public sealed partial class Handler(
    ...
    IUnlockSource unlockSource,
    ISetSourceUnlocked setSourceUnlocked,
...
{
    public DomainEvent EventType { get; } = new SourceUnlockRequested();
    ...
    {
        var unlocked = await unlockSource.HandleAsync(migrationId, ct);
        return await unlocked.ThenAsync(response => response switch
        {
            IUnlockSource.Response.Unlocked => AdvanceAsync(migrationId, ct),
            IUnlockSource.Response.Faulted(var reason) => ScheduleRetryAsync(migrationId, reason, ct),
        });
    }

    private async Task<Result<ValueTuple>> AdvanceAsync(Id migrationId, CancellationToken ct)
    {
        var flagged = await setSourceUnlocked.HandleAsync(migrationId, ct);
    ...
    {
        var scheduled = await scheduleEvent.HandleAsync(migrationId, new SourceUnlockRequested(), settings.MaxAttempts, ct);
    ...
    private Result<ValueTuple> OutOfRetries(Id migrationId, string reason)
    {
        ...
        return new OutOfRetriesFailure(
            $"SourceUnlock exhausted {settings.MaxAttempts} attempts for migration {migrationId.Value}.");
    }
    ...
    private async Task<Result<ValueTuple>> FanInAsync(ISetSourceUnlocked.Response response, CancellationToken ct)
    {
        return response switch
        {
            ISetSourceUnlocked.Response.Complete(var unlocking) => await FinalizeCompletedAsync(unlocking, ct),
            ISetSourceUnlocked.Response.Cancel(var cancelling) => await FinalizeCancelledAsync(cancelling, ct),
            ISetSourceUnlocked.Response.TeardownIncomplete => Done.Result,
        };
    }
    ...
    private static partial class Log
    {
        [LoggerMessage(EventId = LogEvents.StepRetryScheduled, EventName = nameof(LogEvents.StepRetryScheduled),
            Level = LogLevel.Warning, Message = "SourceUnlock faulted for migration {MigrationId}; scheduling attempt {NextAttempt}. Reason: {Reason}")]
        ...
        [LoggerMessage(EventId = LogEvents.StepOutOfRetries, EventName = nameof(LogEvents.StepOutOfRetries),
            Level = LogLevel.Error, Message = "SourceUnlock exhausted {MaxAttempts} attempts for migration {MigrationId}. Reason: {Reason}")]
