...
public sealed partial class NotifyCompletion(IFetch fetchExternalId, ILogger<NotifyCompletion> logger) : INotifyCompletion
{
    ...
    // A switch expression, not a statement: only expressions are exhaustiveness-checked,
    // so a new Request case fails compilation here instead of silently notifying nobody.
    private Result<ValueTuple> Notify(INotifyCompletion.Request request, IdValue pigeonTag)
    {
        return request switch
        {
            INotifyCompletion.Request.Migrated => LogMigrated(pigeonTag),
            INotifyCompletion.Request.Cancelled => LogCancelled(pigeonTag),
        };
    }

    private Result<ValueTuple> LogMigrated(IdValue pigeonTag)
    {
        Log.Migrated(logger, pigeonTag.Value);
        return default(ValueTuple);
    }

    private Result<ValueTuple> LogCancelled(IdValue pigeonTag)
    {
