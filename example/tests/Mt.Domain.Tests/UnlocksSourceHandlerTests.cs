...
using Response = Mt.Domain.Steps.UnlocksSource.IFetchMigration.Response;
using UnlockResponse = Mt.Domain.Steps.UnlocksSource.IUnlockSource.Response;
using SetResponse = Mt.Domain.Steps.UnlocksSource.ISetSourceUnlocked.Response;
...
public class UnlocksSourceHandlerTests
{
    ...
    public UnlocksSourceHandlerTests()
    {
        _fetchMigration.Setup(f => f.HandleAsync(It.IsAny<Id>(), It.IsAny<CancellationToken>()))
            .ReturnsAsync((Response)new Response.Proceed());
        _unlock.Setup(u => u.HandleAsync(It.IsAny<Id>(), It.IsAny<CancellationToken>()))
            .ReturnsAsync((UnlockResponse)new UnlockResponse.Unlocked());
        _setUnlocked.Setup(s => s.HandleAsync(It.IsAny<Id>(), It.IsAny<CancellationToken>()))
            .ReturnsAsync((SetResponse)new SetResponse.TeardownIncomplete());
    ...
    {
        _setUnlocked.Setup(s => s.HandleAsync(It.IsAny<Id>(), It.IsAny<CancellationToken>()))
            .ReturnsAsync((SetResponse)new SetResponse.Complete(new Unlocking(Id(), Org(), true, true)));
    ...
    {
        _setUnlocked.Setup(s => s.HandleAsync(It.IsAny<Id>(), It.IsAny<CancellationToken>()))
            .ReturnsAsync((SetResponse)new SetResponse.Cancel(new Cancelling(Id(), Org(), true, true)));
    ...
    {
        _fetchMigration.Setup(f => f.HandleAsync(It.IsAny<Id>(), It.IsAny<CancellationToken>()))
            .ReturnsAsync((Response)new Response.DoNotProceed());
