...
namespace Mt.Persistence.Steps.UploadsSaft;

/// <summary>
/// Loads the migration and maps it to this step's response (specs 4/5/9): cancellation is a
/// normal skip outcome (logged here), any state other than <see cref="PreSaftUploaded"/> is a
/// failure.
/// </summary>
public sealed partial class FetchMigration(WorkshopDbContext db, ILogger<FetchMigration> logger) : IFetchMigration
{
    ...
    private Result<IFetchMigration.Response> ToResponse(Id migrationId, Migration migration)
    {
        switch (migration)
        {
            ...
            case PreSaftUploaded preSaftUploaded:
                return new IFetchMigration.Response.Proceed(preSaftUploaded);
            default:
                return new MigrationHasIncorrectState(
                    $"SaftUpload expected migration {migrationId.Value} to be PreSaftUploaded but was {migration.GetType().Name}.");
        }
    }
    ...
    private static partial class Log
    {
        [LoggerMessage(EventId = LogEvents.StepSkippedCancelled, EventName = nameof(LogEvents.StepSkippedCancelled),
            Level = LogLevel.Warning, Message = "Skipping SaftUpload for cancelled migration {MigrationId}.")]
